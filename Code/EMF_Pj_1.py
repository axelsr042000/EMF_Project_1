...
########################################################################################################################
"""
CREATION DATAFRAME POUR CALCUL DAILY RETURN
"""
...
# CALCULER LES RENDEMENTS QUOTIDIENS A L'AIDE DES VALEURS DECALEES
...
########################################################################################################################
"""
CREATION DATAFRAME POUR CALCUL WEEKLY RETURN
"""
...
lagged_index = (indices_df.index + DateOffset(days=-1)).to_period('D')
indices_df.index = indices_df.index.to_period('D')
indices_ret_df = np.log(indices_df).values - np.log(indices_df.reindex(lagged_index)).values
indices_ret_df = pd.DataFrame(indices_ret_df, index=indices_df.index, columns=indices_df.columns)
'''
########################################################################################################################
